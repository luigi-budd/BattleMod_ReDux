...
CR.ChaosRingCapHUD = function(v)
	...
	if not(F.GameState.CaptureHUDTimer) then --... Except for the text easing in.
		...
		if F.GameState.CaptureHUDName > 0 then
			...
			lerpamt = B.FixedLerp(0,FRACUNIT,$*9/10)
	...
	if not(#server.AvailableChaosRings or B.PreRoundWait()) then
		...
		if tics > TICRATE or tics%2!=0 then
			for i = 1, 6 do
				...
				v.drawScaled(
					x + FixedMul(cos(angle), radius) - xoff,
					y + FixedMul(sin(angle), radius),
					FRACUNIT/scalediv,
					v.cachePatch("CHRING"..i),
					f
				)
			end
		end
...
CR.ChaosRingHUD = function(v, player)
	...
	local y = 26*FRACUNIT
	for t = 1, 2 do
		local flip = (t==1 and 1) or -1
		for num = 1, 6 do
			local chaosring = CR.GetChaosRing(num)

			if not(chaosring and chaosring.valid) then continue end
			if not(
				(chaosring.captured and (chaosring.captureteam == t))
				or ((
					chaosring.target and chaosring.target.valid and chaosring.target.player
					and (chaosring.target.player.ctfteam == t)
					and chaosring.target.player.gotcrystal_time
					and not(chaosring.chaosring_bankkey))
					and (leveltime%2)==1
				)
			) then continue end
			...
			v.drawScaled(
				(x-(FRACUNIT*3)+(flip*((FRACUNIT/3)*(28)))*(i+2)),
				(y+(FRACUNIT*10)),
				FRACUNIT/2,
				patch, flags|trans
			)
...
CR.BankRankHUD = function(v)
	...
	for i=1, #players_sorted do
		...
		local fy = cond and y+11 or y+8

		if p.gotcrystal and p.mo and p.mo.valid and p.mo.chaosring and p.mo.chaosring.valid then
			local intpatch = {v.getSpritePatch(SPR_TRNG, p.mo.chaosring.frame)}
